...
@interface RBPhotoBrowser () <UIScrollViewDelegate, UIViewControllerTransitioningDelegate, CAAnimationDelegate> {
...
+ (instancetype)browserWithPhotoItems:(NSArray<RBPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    RBPhotoBrowser *browser = [[RBPhotoBrowser alloc] initWithPhotoItems:photoItems selectedIndex:selectedIndex];
    return browser;
}
...
- (instancetype)initWithPhotoItems:(NSArray<RBPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    ...
    if (self) {
        ...
        _imageManager = [[RBSDImageManager alloc] init];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    rect.origin.x -= kRBPhotoViewPadding;
    rect.size.width += 2 * kRBPhotoViewPadding;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    RBPhotoItem *item = [_photoItems objectAtIndex:_currentPage];
    RBPhotoView *photoView = [self photoViewForPage:_currentPage];
...
- (RBPhotoView *)photoViewForPage:(NSUInteger)page {
    for (RBPhotoView *photoView in _visibleItemViews) {
...
- (RBPhotoView *)dequeueReusableItemView {
    RBPhotoView *photoView = [_reusableItemViews anyObject];
    if (photoView == nil) {
        photoView = [[RBPhotoView alloc] initWithFrame:_scrollView.bounds imageManager:_imageManager];
    } else {
...
- (void)configItemViews {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) {
        ...
        RBPhotoView *photoView = [self photoViewForPage:i];
        if (photoView == nil) {
        ...
        if (photoView.item == nil && _presented) {
            RBPhotoItem *item = [_photoItems objectAtIndex:i];
            [self configPhotoView:photoView withItem:item];
        }
    }
...
- (void)configPhotoView:(RBPhotoView *)photoView withItem:(RBPhotoItem *)item {
